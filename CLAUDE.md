...
2. **Built-in plugins** — `src/widgets/plugins/<name>/{index.ts,component.tsx,types.ts}`.
   ...
   - Definitions: `src/widgets/plugins-generated.ts` is a static barrel (auto-generated by
     `scripts/generate-plugin-barrel.mjs`, run via `predev`/`prebuild` and `create-widget`)
     listing `{ dir, widget }` for each `plugins/<name>/index.ts`. `widgets.js` imports it and
     passes it to `getPluginDefinitions` (`plugin-loader.ts` `loadBuiltinPlugins`). Static
     imports are required (NOT `require.context`): built-ins are webpack async modules, so
     `require.context` returns Promises that the synchronous loader can't use.
   - Components loaded by `src/widgets/plugin-components.ts` via
     `require.context("./plugins", true, /component\.(tsx|jsx)$/, "lazy")` (components are
     `next/dynamic`, so Promises are fine there).
3. **External plugins** — dropped into `HOMEPAGE_PLUGINS_DIR` at runtime, no rebuild.
   - Definitions ALSO loaded by `plugin-loader.ts` (`loadExternalPlugins`, via
     `__non_webpack_require__` so webpack never tries to bundle the arbitrary path; tests
     supply it through `vitest.setup.js` via `createRequire`).
...
- Built-in definitions: imported statically from the generated barrel
  `src/widgets/plugins-generated.ts` (see "Built-in plugins" above). `require.context` does
  NOT work for these: built-ins are webpack async modules (they import proxy handlers), and
  `require.context` returns Promises for async modules, unusable in the synchronous
  `getPluginDefinitions`. The barrel uses static imports, resolved by webpack's async-module
  runtime before `getPluginDefinitions` runs (same mechanism as `legacy-widgets.js`). The
  barrel is imported by `widgets.js` (not `plugin-loader.ts`) to keep `plugin-loader` out of
  the `plugins -> generic -> widgets -> plugin-loader` import cycle.
- External definitions: `loadExternalPlugins` loads each `HOMEPAGE_PLUGINS_DIR/*/index.js`
  with `__non_webpack_require__` (the real Node require inside a webpack bundle; webpack never
  bundles arbitrary paths). Tests provide it via `vitest.setup.js` (`createRequire`).
- `dev` uses `next dev --webpack` (package.json) so `require.context` (still used for
  component loading) and `__non_webpack_require__` behave the same in dev and prod.
- Regenerate the barrel with `node scripts/generate-plugin-barrel.mjs` (automatic on
  `predev`/`prebuild` and inside `pnpm create-widget`). The generated file is committed.
